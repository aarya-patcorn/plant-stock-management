        ...
        <table className="min-w-[720px] border-separate border-spacing-0 sm:min-w-full">
            ...
            {table.getHeaderGroups().map((headerGroup) => (
              <tr key={headerGroup.id}>
                {headerGroup.headers.map((header) => {
                  ...
                  return (
                    <th
                      className="whitespace-nowrap border-b border-slate-800 px-3 py-2.5 text-left text-[11px] font-semibold uppercase tracking-wide sm:px-4 sm:py-3 sm:text-xs"
                      key={header.id}
                    >
          ...
          <tbody>
            {isLoading ? (
              <tr>
                <td
                  className="px-3 py-6 text-center text-xs text-slate-500 sm:px-4 sm:py-8 sm:text-sm"
              ...
              table.getRowModel().rows.map((row) => (
                ...
                >
                  {row.getVisibleCells().map((cell) => (
                    <td
                      className="whitespace-nowrap border-b border-slate-100 px-3 py-2.5 text-xs text-slate-700 align-top sm:px-4 sm:py-3 sm:text-sm"
                      key={cell.id}
                    >
