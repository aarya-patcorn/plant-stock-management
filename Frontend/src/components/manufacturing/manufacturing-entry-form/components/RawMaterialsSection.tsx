...
interface RawMaterialsSectionProps {
  ...
  updateRawMaterialTextNumberField: (index: number, field: string, value: string) => void;
}
...
export function RawMaterialsSection({
  ...
  updateRawMaterialTextNumberField,
    ...
    <div className="space-y-4 rounded-2xl border border-slate-200/80 bg-slate-50/70 p-4 shadow-sm">
      ...
      {rawMaterials.map((item, index) => (
        ...
        >
          ...
          <Field className="min-w-0" htmlFor={`level2-${index}`} label="Type Of Raw Material">
            <Input
              className="w-full"
              id={`level2-${index}`}
              placeholder=""
              value={item.level2}
              onChange={(e) =>
                updateRawMaterialTextNumberField(index, "level2", e.target.value)
              }
            />
          </Field>

          {item.level3?.trim() && (
            <Field className="min-w-0" htmlFor={`level3-${index}`} label="Material Variant">
              <Input
                className="w-full"
                id={`level3-${index}`}
